...
//       }).join('');
    ...
    if ( 0 == Array.from(reportTables.entries()).reduce((total, value) => { return total + value.rows.length}, 0) ) {
  ...
  } catch (error) { return message.reply(`There was an error: ${error.trace}\n${errors}`); } 
};
